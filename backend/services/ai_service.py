        ...
        try:
            ...
            if resume_text and resume_text.strip():
                ...
                if is_default_fake_resume and not resume_text.count('\n') > 3:
                    # 기본 가짜 이력서인 경우 (실사용 텍스트가 사실상 없을 때)
                    # 출력물에 메타/상황 설명이 노출되지 않도록, 간결히 전달
...
자료가 제공되지 않았습니다."""
            ...
            # System Prompt: 사고 과정 준수 + 결과물에 메타/지시문 금지 강화
            prompt = f"""<|system|>
...
- **절대, 절대, 절대** 메타/설명형 문장을 쓰지 마세요. 다음 문구들은 절대 포함하지 마세요:
  ❌ "(자료가 없어, 일반적인 답변을 생성합니다.)"
  ❌ "(자료가 없어...)"
  ❌ "(이력서 내용이 없으므로...)"
  ❌ "(일반적인 상황을 가정하여...)"
  ❌ "(구체적인 경험이 없으므로...)"
...
- 자료 유무와 관계없이 자연스럽고 완성도 높은 자기소개서를 작성하세요. 메타 정보나 상황 설명 없이 바로 본론으로 들어가세요.
